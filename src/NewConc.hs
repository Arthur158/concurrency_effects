...
data EndOfProgram k = EndProg Bool k
  deriving Functor

hEndOfProgram :: Functor f' => Handler EndOfProgram a f' a
hEndOfProgram = Handler
  { ret = pure
    , hdlr = \x -> case x of
                     EndProg b k -> k
  }
...
swap' :: (Functor f, Choose <: f)=> Free (EndOfProgram + f) (a, b) -> Free (EndOfProgram + f) (b, a)
swap' (Pure (x,y)) = Pure (y, x)
swap' (Op f) = case f of
                 R _ -> Op (fmap swap' f)
                 L (EndProg b k) -> Op (inj' (EndProg (not b) (swap' k)))


-- newpar :: (Choose <: f) => Free (f) a -> Free (f) b -> Free f (a, b)
-- newpar a b = handle hEndOfProgram (par' (mask a) (mask b))

par' :: (Choose <: f) => Free (EndOfProgram + f) a -> Free (EndOfProgram + f) b -> Free (EndOfProgram + f) (a, b)
par' program1 program2 = do
  goesFirst program1 program2 ~+~ swap' (goesFirst program2 program1)

goesFirst :: (Choose <: f) => Free (EndOfProgram + f) a -> Free (EndOfProgram + f) b -> Free (EndOfProgram + f) (a, b)
goesFirst (Pure x) y = Op (inj' (EndProg False (fmap (x,) y)))
